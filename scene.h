...
public:
	...
	void updatePosition(double timeStep)
	{
		...
		// (0, \omega)
		RowVector4d tempQ; tempQ << 0, angVelocity;
		orientation += 0.5 * timeStep * QMult(tempQ, orientation);
	...
	void updateVelocity(double timeStep)
	{
		...
		Vector3d gravity; gravity << 0, -9.8, 0;
...
public:
	...
	bool loadScene(const std::string dataFolder, const std::string sceneFileName, const std::string constraintFileName)
	{
		...
		for (int i = 0; i < numofConstraints; i++)
		{
			...
			constraints.push_back(Constraint(ConstraintType::STRETCH, ConstraintEqualityType::DOUBLE_SIDED_INEQUALITY, attachM1, attachV1, attachM2, attachV2, invMass1, invMass2, RowVector3d::Zero(), initDist, .2, 0.0));
